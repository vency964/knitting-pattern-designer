...
const T = {
  bg: {
    ...
    pdfTitle:"Схема за плетене",pdfLegend:"Легенда на символите",pdfInstructions:"Инструкции",
    pdfGenerated:"PDF генериран!",pdfInfo:"Информация за проекта",pdfGauge:"Гейдж",
  },
  en: {
    ...
    pdfTitle:"Knitting Pattern",pdfLegend:"Stitch Legend",pdfInstructions:"Instructions",
    pdfGenerated:"PDF generated!",pdfInfo:"Project info",pdfGauge:"Gauge",
  },
};
...
const STITCH_SETS = {
  crochet: [
    ...
    {id:"hdc",label:"Half dbl",labelBg:"Полустълбче",sym:"T",color:"#1d9e75",desc:"Medium height stitch",descBg:"Среден по височина"},
    {id:"dc",label:"Double cr.",labelBg:"Стълбче",sym:"╤",color:"#d85a30",desc:"Tall stitch",descBg:"Висок бод"},
    {id:"tr",label:"Treble",labelBg:"Дв. стълбче",sym:"╦",color:"#993556",desc:"Extra tall stitch",descBg:"Много висок бод"},
    {id:"dtr",label:"Dbl treble",labelBg:"Тр. стълбче",sym:"╬",color:"#7f77dd",desc:"Double treble",descBg:"Тройно стълбче"},
    {id:"inc",label:"Increase",labelBg:"Увеличение",sym:"V",color:"#639922",desc:"2 stitches in 1",descBg:"2 бода в 1"},
    {id:"dec",label:"Decrease",labelBg:"Намаление",sym:"Λ",color:"#a32d2d",desc:"2 stitches together",descBg:"2 бода заедно"},
    ...
    {id:"sh",label:"Shell",labelBg:"Мида",sym:"⌓",color:"#185fa5",desc:"Fan/shell group",descBg:"Група мида"},
    {id:"pic",label:"Picot",labelBg:"Пико",sym:"⌃",color:"#0f6e56",desc:"Decorative loop",descBg:"Декоративна бримка"},
    {id:"fpdc",label:"FPdc",labelBg:"ЛП стълбче",sym:"⫰",color:"#534ab7",desc:"Front post dc",descBg:"Лицево стълбче"},
    {id:"bpdc",label:"BPdc",labelBg:"ЗП стълбче",sym:"⫯",color:"#3c3489",desc:"Back post dc",descBg:"Задно стълбче"},
    {id:"puff",label:"Puff",labelBg:"Пуф",sym:"◉",color:"#854f0b",desc:"Puff stitch",descBg:"Пуф бод"},
  ],
  knitting: [
    ...
    {id:"yo",label:"Yarn over",labelBg:"Обвивка",sym:"○",color:"#1d9e75",desc:"Yarn over needle",descBg:"Обвивка на игла"},
    ...
    {id:"sl1",label:"Slip 1",labelBg:"Прехвърли 1",sym:"▽",color:"#888",desc:"Slip 1 stitch",descBg:"Прехвърли 1 бод"},
    {id:"cab4f",label:"Cable 4F",labelBg:"Плитка 4Л",sym:"⟨⟩",color:"#534ab7",desc:"Cable 4 front",descBg:"Плитка 4 лицева"},
    {id:"cab4b",label:"Cable 4B",labelBg:"Плитка 4З",sym:"⟩⟨",color:"#3c3489",desc:"Cable 4 back",descBg:"Плитка 4 задна"},
    {id:"m1l",label:"M1L",labelBg:"Д1Л",sym:"↗",color:"#639922",desc:"Make 1 left",descBg:"Добави 1 ляво"},
    {id:"m1r",label:"M1R",labelBg:"Д1Д",sym:"↖",color:"#0f6e56",desc:"Make 1 right",descBg:"Добави 1 дясно"},
    {id:"bo",label:"Bind off",labelBg:"Закриване",sym:"⌐",color:"#555",desc:"Bind off stitch",descBg:"Закриващ бод"},
    {id:"co",label:"Cast on",labelBg:"Набиране",sym:"⌐",color:"#ba7517",desc:"Cast on stitch",descBg:"Набиращ бод"},
  ],
  crossStitch: [
    {id:"full",label:"Full ×",labelBg:"Пълен ×",sym:"×",color:"#2a7fff",desc:"Full cross stitch",descBg:"Пълен кръстат бод"},
    ...
    {id:"french",label:"French knot",labelBg:"Фр. възел",sym:"●",color:"#993556",desc:"French knot",descBg:"Френски възел"},
...
function quantizeColors(pixels,num){if(!pixels.length)return[];const bk=[pixels.slice()];while(bk.length<num){let wi=0,wr=-1;for(let i=0;i<bk.length;i++){const b=bk[i];if(b.length<2)continue;let mnR=255,mxR=0,mnG=255,mxG=0,mnB=255,mxB=0;for(const[r,g,bl]of b){if(r<mnR)mnR=r;if(r>mxR)mxR=r;if(g<mnG)mnG=g;if(g>mxG)mxG=g;if(bl<mnB)mnB=bl;if(bl>mxB)mxB=bl;}const range=Math.max(mxR-mnR,mxG-mnG,mxB-mnB);if(range>wr){wr=range;wi=i;}}const b=bk[wi];if(b.length<2)break;let mnR=255,mxR=0,mnG=255,mxG=0,mnB=255,mxB=0;for(const[r,g,bl]of b){if(r<mnR)mnR=r;if(r>mxR)mxR=r;if(g<mnG)mnG=g;if(g>mxG)mxG=g;if(bl<mnB)mnB=bl;if(bl>mxB)mxB=bl;}const rR=mxR-mnR,rG=mxG-mnG,rB=mxB-mnB;const ch=rR>=rG&&rR>=rB?0:rG>=rB?1:2;b.sort((a,c)=>a[ch]-c[ch]);const mid=Math.floor(b.length/2);bk.splice(wi,1,b.slice(0,mid),b.slice(mid));}return bk.map(b=>{let sr=0,sg=0,sb=0;for(const[r,g,bl]of b){sr+=r;sg+=g;sb+=bl;}const n=b.length;return"#"+[Math.round(sr/n),Math.round(sg/n),Math.round(sb/n)].map(v=>v.toString(16).padStart(2,"0")).join("");});}
...
export default function KnittingDesigner(){
  ...
  // Gauge calculator
  const gaugeResult=useMemo(()=>{if(!gaugeStPer10||!gaugeRowPer10)return null;const castOn=Math.round(targetW/10*gaugeStPer10);const totalRows=Math.round(targetH/10*gaugeRowPer10);return{castOn,totalRows};},[targetW,targetH,gaugeStPer10,gaugeRowPer10]);

  // PDF Export
  const exportPDF=()=>{
    const pdf=new jsPDF("p","mm","a4");const pw=210,ph=297;let y=15;
    pdf.setFontSize(18);pdf.text(projectName,pw/2,y,{align:"center"});y+=8;
    ...
    pdf.text(`${craftLabel} | ${gridW}×${gridH} | ${t.stitchCount}: ${stitchCount}`,pw/2,y,{align:"center"});y+=4;
    if(yarnEstimate)pdf.text(`${t.estimatedYarn}: ~${yarnEstimate.totalM}${t.meters} (${yarnEstimate.balls} ${t.balls})`,pw/2,y,{align:"center"});
    y+=8;pdf.setTextColor(0);
    // Draw chart
    const maxChartW=pw-20,maxChartH=120;const cs=Math.min(maxChartW/gridW,maxChartH/gridH,4);
    ...
    for(let gy=0;gy<gridH;gy++)for(let gx=0;gx<gridW;gx++){
      ...
      if(s&&cs>=2.5){pdf.setFontSize(cs*1.8);pdf.setTextColor(0);pdf.text(s.sym,cx+gx*cs+cs/2,y+gy*cs+cs*0.7,{align:"center"});}
      pdf.setDrawColor(220);pdf.rect(cx+gx*cs,y+gy*cs,cs,cs);
    }
    // Row numbers
    pdf.setFontSize(5);pdf.setTextColor(150);
    for(let gy=0;gy<gridH;gy+=5)pdf.text(String(gy+1),cx-2,y+gy*cs+cs*0.7,{align:"right"});
    y+=chartH+8;
    // Legend
    if(usedStitches.length>0){
      pdf.setFontSize(12);pdf.setTextColor(0);pdf.text(t.pdfLegend,10,y);y+=6;
      pdf.setFontSize(8);
      usedStitches.forEach((s,i)=>{if(y>ph-20){pdf.addPage();y=15;}
        pdf.setFontSize(12);pdf.text(s.sym,14,y);
        pdf.setFontSize(8);pdf.text(`${lang==="bg"?s.labelBg:s.label} — ${lang==="bg"?s.descBg:s.desc}`,22,y);y+=5;});
      y+=4;
    }
    // Instructions
    if(y>ph-40){pdf.addPage();y=15;}
    pdf.setFontSize(12);pdf.setTextColor(0);pdf.text(t.pdfInstructions,10,y);y+=6;
    pdf.setFontSize(7);pdf.setTextColor(60);
    const instrText=generateInstructions();instrText.split("\n").forEach(line=>{if(y>ph-10){pdf.addPage();y=15;}pdf.text(line,10,y);y+=4;});
    ...
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",height:"100vh",background:"var(--color-background-tertiary)",color:"var(--color-text-primary)",overflow:"hidden"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 12px",background:"var(--color-background-primary)",borderBottom:"1px solid var(--color-border-tertiary)",flexWrap:"wrap",gap:"6px"}}>
        ...
        <div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <button style={{...B(false),borderColor:"#D4537E",background:"#D4537E10"}} onClick={()=>setShowLegend(true)}>{"📋 "+t.legend}</button>
          <button style={{...B(false),borderColor:"#185FA5",background:"#185FA510"}} onClick={()=>setShowGauge(true)}>{"🔢 "+t.gaugeCalc}</button>
          <span style={{width:1,height:20,background:"var(--color-border-tertiary)"}}/>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        <div style={{width:220,background:"var(--color-background-primary)",borderRight:"1px solid var(--color-border-tertiary)",display:"flex",flexDirection:"column",overflow:"auto",flexShrink:0}}>
          ...
          {panel==="tools"&&<div style={SS}><div style={ST}>{t.tools}</div><div style={{display:"flex",flexDirection:"column",gap:4}}>
              ...
              {selRect&&<div style={{display:"flex",flexWrap:"wrap",gap:3}}>
                ...
                <button style={SB(false)} onClick={flipH}>{t.flipH}</button>
                <button style={SB(false)} onClick={flipV}>{t.flipV}</button>
                <button style={SB(false)} onClick={rotateSel}>{t.rotate}</button>
              </div>}
              ...
              <button style={B(false)} onClick={undo}>{t.undo}</button><button style={B(false)} onClick={redo}>{t.redo}</button>
              <button style={B(false)} onClick={clearAll}>{t.clear}</button>
              <div style={{height:4}}/>
              <div style={ST}>{t.gridW}</div><input type="number" style={NI} value={gridW} min={5} max={100} onChange={e=>resizeGrid(+e.target.value||5,gridH)}/>
              ...
              <div style={{height:4}}/>
              <div style={ST}>{t.zoom}</div><input type="range" min={0.5} max={3} step={0.1} value={zoom} onChange={e=>setZoom(+e.target.value)} style={{width:"100%"}}/><span style={{fontSize:11,color:"var(--color-text-tertiary)"}}>{Math.round(zoom*100)}%</span>
              <div style={{height:4}}/>
              <div style={ST}>{t.repeatDetect}</div>
              {repeatInfo?<div style={{padding:"4px 8px",background:"var(--color-background-info)",borderRadius:6,fontSize:11,color:"var(--color-text-info)"}}>{t.repeatFound}: {repeatInfo.rw}×{repeatInfo.rh}</div>:<span style={{fontSize:11,color:"var(--color-text-tertiary)"}}>{t.repeatNone}</span>}
              <div style={{height:4}}/>
          ...
          <div style={{...SS,marginTop:"auto"}}><div style={{display:"flex",flexDirection:"column",gap:4}}>
            ...
            <div style={{display:"flex",gap:4}}><button style={{...B(false),flex:1}} onClick={exportPNG}>{t.exportPNG}</button><button style={{...B(false),flex:1}} onClick={exportSVG}>{t.exportSVG}</button><button style={{...B(false),flex:1,borderColor:"#D4537E",color:"#D4537E"}} onClick={exportPDF}>{t.exportPDF}</button></div>
        ...
        <div style={{flex:1,overflow:"auto",display:"flex",flexDirection:"column"}}>
          <div style={{display:"flex",gap:4,padding:"6px 8px",background:"var(--color-background-secondary)",borderBottom:"1px solid var(--color-border-tertiary)",flexWrap:"wrap",alignItems:"center"}}>
            ...
            <span style={{flex:1}}/><span style={{fontSize:11,color:"var(--color-text-tertiary)"}}>{t.stitchCount}: {stitchCount} | {gridW}{t.cols} × {gridH}{t.rows}{repeatInfo?` | ${t.repeatFound}: ${repeatInfo.rw}×${repeatInfo.rh}`:""}</span>
          </div>
          <div style={{flex:1,overflow:"auto",padding:12,display:"flex",justifyContent:"center",alignItems:"flex-start"}}>
            <div ref={gridRef} style={{position:"relative",cursor:isPasting?"copy":tool==="select"?"crosshair":tool==="eraser"?"crosshair":"crosshair",userSelect:"none",touchAction:"none"}}
      ...
      <div style={{display:"flex",gap:12,padding:"4px 10px",fontSize:11,color:"var(--color-text-tertiary)",background:"var(--color-background-primary)",borderTop:"1px solid var(--color-border-tertiary)"}}>
        <span>{projectName}</span><span>{craftType==="crochet"?t.crochet:craftType==="knitting"?t.knitting:t.crossStitch}</span><span>{gridW}×{gridH}</span><span>{t.progress}: {progressRow}/{gridH}</span>{yarnEstimate&&<span>~{yarnEstimate.totalM}{t.meters}</span>}<span style={{marginLeft:"auto"}}>{t.stitchCount}: {stitchCount}</span></div>

      {/* Legend Modal */}
      {showLegend&&<div style={MOD} onClick={()=>setShowLegend(false)}><div style={MC} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <h3 style={{margin:0,fontSize:17,fontWeight:600}}>{"📋 "+t.legendTitle}</h3>
          <button style={B(false)} onClick={()=>setShowLegend(false)}>{t.close}</button></div>
        {usedStitches.length===0?<div style={{padding:20,textAlign:"center",color:"var(--color-text-tertiary)"}}>{lang==="bg"?"Няма използвани бодове":"No stitches used"}</div>:
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
          <thead><tr style={{borderBottom:"2px solid var(--color-border-tertiary)"}}>
            <th style={{padding:"8px 12px",textAlign:"left",fontWeight:600}}>{t.symbol}</th>
            <th style={{padding:"8px 12px",textAlign:"left",fontWeight:600}}>{t.stitch}</th>
            <th style={{padding:"8px 12px",textAlign:"left",fontWeight:600}}>{t.description}</th>
          </tr></thead>
          <tbody>{usedStitches.map(s=>(<tr key={s.id} style={{borderBottom:"1px solid var(--color-border-tertiary)"}}>
            <td style={{padding:"8px 12px",fontSize:20,color:s.color,textAlign:"center"}}>{s.sym}</td>
            <td style={{padding:"8px 12px",fontWeight:500}}>{lang==="bg"?s.labelBg:s.label}</td>
            <td style={{padding:"8px 12px",color:"var(--color-text-secondary)"}}>{lang==="bg"?s.descBg:s.desc}</td>
          </tr>))}</tbody>
        </table>}
      </div></div>}

      {/* Gauge Calculator Modal */}
      {showGauge&&<div style={MOD} onClick={()=>setShowGauge(false)}><div style={{...MC,maxWidth:500}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <h3 style={{margin:0,fontSize:17,fontWeight:600}}>{"🔢 "+t.gaugeTitle}</h3>
          <button style={B(false)} onClick={()=>setShowGauge(false)}>{t.close}</button></div>
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <div><div style={ST}>{t.stitchesPer10cm}</div><input type="number" style={{...NI,width:"100%"}} value={gaugeStPer10} onChange={e=>setGaugeStPer10(+e.target.value||1)}/></div>
            <div><div style={ST}>{t.rowsPer10cm}</div><input type="number" style={{...NI,width:"100%"}} value={gaugeRowPer10} onChange={e=>setGaugeRowPer10(+e.target.value||1)}/></div>
          </div>
          <div style={{height:1,background:"var(--color-border-tertiary)"}}/>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <div><div style={ST}>{t.targetWidth}</div><input type="number" style={{...NI,width:"100%"}} value={targetW} onChange={e=>setTargetW(+e.target.value||1)}/></div>
            <div><div style={ST}>{t.targetHeight}</div><input type="number" style={{...NI,width:"100%"}} value={targetH} onChange={e=>setTargetH(+e.target.value||1)}/></div>
          </div>
          {gaugeResult&&<div style={{padding:16,background:"var(--color-background-info)",borderRadius:10,display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            <div style={{textAlign:"center"}}><div style={{fontSize:28,fontWeight:600,color:"var(--color-text-info)"}}>{gaugeResult.totalRows}</div><div style={{fontSize:11,color:"var(--color-text-tertiary)"}}>{t.totalRows}</div></div>
            <div style={{gridColumn:"1/3",textAlign:"center",fontSize:12,color:"var(--color-text-secondary)"}}>{t.finishedSize}: {targetW}cm × {targetH}cm</div>
          </div>}
          {yarnEstimate&&<div style={{padding:12,background:"var(--color-background-secondary)",borderRadius:8,textAlign:"center"}}>
            <div style={{fontSize:11,color:"var(--color-text-tertiary)",marginBottom:4}}>{t.estimatedYarn}</div>
            <div style={{fontSize:18,fontWeight:600,color:"var(--color-text-info)"}}>~{yarnEstimate.totalM}{t.meters} ({yarnEstimate.balls} {t.balls})</div>
          </div>}
        </div>
      </div></div>}
      ...
      {showTemplates&&<div style={MOD} onClick={()=>setShowTemplates(false)}><div style={MC} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}><h3 style={{margin:0,fontSize:17,fontWeight:600}}>{"📐 "+t.templateTitle}</h3><button style={B(false)} onClick={()=>setShowTemplates(false)}>{t.close}</button></div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
          {Object.entries(GARMENT_TEMPLATES).map(([key,tmpl])=>(<div key={key} onClick={()=>applyTemplate(key)} style={{padding:16,borderRadius:10,border:"1px solid var(--color-border-tertiary)",background:"var(--color-background-secondary)",cursor:"pointer",textAlign:"center"}}>
            <div style={{fontSize:28,marginBottom:6}}>{key==="sweater"?"👕":key==="hat"?"🧢":key==="scarf"?"🧣":key==="sock"?"🧦":"🛏️"}</div>
            <div style={{fontSize:14,fontWeight:500}}>{lang==="bg"?tmpl.labelBg:tmpl.label}</div><div style={{fontSize:11,color:"var(--color-text-tertiary)",marginTop:4}}>{tmpl.w}×{tmpl.h}</div>
          </div>))}</div></div></div>}

      {/* Library */}
      {showLibrary&&<div style={MOD} onClick={()=>setShowLibrary(false)}><div style={MC} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}><h3 style={{margin:0,fontSize:17,fontWeight:600}}>{"📚 "+t.motifLibrary}</h3><button style={B(false)} onClick={()=>setShowLibrary(false)}>{t.close}</button></div>
        <div style={{display:"flex",gap:8,marginBottom:16,padding:12,background:"var(--color-background-secondary)",borderRadius:8}}>
          ...
          <button style={{...B(true),padding:"6px 16px"}} onClick={saveCurrentAsMotif}>{t.saveMotif}</button></div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(120px,1fr))",gap:10}}>
          {motifs.map((m,i)=>(<div key={i} style={{padding:8,borderRadius:8,border:"1px solid var(--color-border-tertiary)",background:"var(--color-background-secondary)",textAlign:"center"}}>
            <svg width={Math.min(m.w*3,100)} height={Math.min(m.h*3,60)} viewBox={`0 0 ${m.w*CELL} ${m.h*CELL}`} style={{background:"#fff",borderRadius:4,border:"1px solid var(--color-border-tertiary)",marginBottom:4}}>
            ...
            <div style={{display:"flex",gap:3,marginTop:4,justifyContent:"center"}}><button style={SB(true)} onClick={()=>loadMotif(m)}>{t.loadMotif}</button><button style={{...SB(false),color:"var(--color-text-danger)"}} onClick={()=>deleteMotif(i)}>{t.deleteMotif}</button></div>
          </div>))}</div>}
      </div></div>}

      {/* Import */}
      {showImport&&<div style={MOD} onClick={cancelImport}><div style={{...MC,maxWidth:750}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}><h3 style={{margin:0,fontSize:17,fontWeight:600}}>{"📷 "+t.importTitle}</h3><button style={B(false)} onClick={cancelImport}>{t.cancelImport}</button></div>
        {!importImg?(<div onDrop={e=>{e.preventDefault();handleImageFile(e.dataTransfer?.files?.[0]);}} onDragOver={e=>e.preventDefault()} onClick={()=>importFileRef.current?.click()} style={{border:"2px dashed var(--color-border-secondary)",borderRadius:12,padding:"48px 24px",textAlign:"center",cursor:"pointer",background:"var(--color-background-secondary)"}}><div style={{fontSize:48,marginBottom:12,opacity:0.4}}>🖼️</div><div style={{fontSize:14,color:"var(--color-text-secondary)"}}>{t.dropImage}</div><input ref={importFileRef} type="file" accept="image/*" style={{display:"none"}} onChange={e=>handleImageFile(e.target.files?.[0])}/></div>
          ...
          <div style={{display:"flex",gap:12,marginBottom:16,flexWrap:"wrap",alignItems:"center",padding:"10px 12px",background:"var(--color-background-secondary)",borderRadius:8}}>
            <div><label style={{fontSize:11,fontWeight:600,color:"var(--color-text-tertiary)"}}>{t.widthCells}</label><input type="range" min={10} max={120} value={importW} onChange={e=>setImportW(+e.target.value)} style={{width:100,display:"block"}}/><span style={{fontSize:11}}>{importW}×{importH}</span></div>
          ...
          <div style={{display:"flex",gap:16,justifyContent:"center",flexWrap:"wrap"}}>
            ...
            <div style={{textAlign:"center"}}><div style={{fontSize:11,fontWeight:600,color:"var(--color-text-tertiary)",marginBottom:6}}>{t.resultPreview}</div>
              {importPreviewGrid&&!importProcessing?<svg width={importW*ipcs} height={importH*ipcs} style={{borderRadius:8,border:"1px solid var(--color-border-tertiary)",background:"#fff"}}>{importPreviewGrid.map((row,y)=>row.map((c,x)=>c?<rect key={`${x}-${y}`} x={x*ipcs} y={y*ipcs} width={ipcs} height={ipcs} fill={c}/>:null))}</svg>
              :<div style={{width:250,height:170,display:"flex",alignItems:"center",justifyContent:"center",borderRadius:8,border:"1px solid var(--color-border-tertiary)",background:"var(--color-background-secondary)",color:"var(--color-text-tertiary)"}}>{t.processing}</div>}</div></div>
      ...
      {showPreview&&<div style={MOD} onClick={()=>setShowPreview(false)}><div style={MC} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><h3 style={{margin:0,fontSize:16}}>{t.preview}: {projectName}</h3><button style={B(false)} onClick={()=>setShowPreview(false)}>{t.close}</button></div>
        <div style={{textAlign:"center",overflow:"auto"}}><svg width={Math.min(gridW*12,560)} height={Math.min(gridH*12,400)} viewBox={`0 0 ${gridW*CELL} ${gridH*CELL}`} style={{background:"#fff",borderRadius:8,border:"1px solid var(--color-border-tertiary)"}}>{grid.map((row,y)=>row.map((cell,x)=>{const c=colorGrid[y]?.[x];if(c)return<rect key={`p${x}-${y}`} x={x*CELL} y={y*CELL} width={CELL} height={CELL} fill={c}/>;if(cell)return<rect key={`p${x}-${y}`} x={x*CELL} y={y*CELL} width={CELL} height={CELL} fill={cell.color} opacity={0.3}/>;return null;}))}</svg></div></div></div>}

      {showInstructions&&<div style={MOD} onClick={()=>setShowInstructions(false)}><div style={MC} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><h3 style={{margin:0,fontSize:16}}>{t.instructions}</h3><div style={{display:"flex",gap:6}}><button style={B(false)} onClick={()=>navigator.clipboard.writeText(generateInstructions())}>Copy</button><button style={B(false)} onClick={()=>setShowInstructions(false)}>{t.close}</button></div></div>
        <pre style={{fontSize:12,lineHeight:1.6,whiteSpace:"pre-wrap",background:"var(--color-background-secondary)",padding:12,borderRadius:8,maxHeight:400,overflow:"auto",fontFamily:"var(--font-mono,monospace)"}}>{generateInstructions()}</pre></div></div>}
    </div>
  );
}
